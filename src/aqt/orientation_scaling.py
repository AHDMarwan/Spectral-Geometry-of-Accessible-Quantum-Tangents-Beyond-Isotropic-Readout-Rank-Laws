...
def _fit_plateau_inv_n(n: np.ndarray, g: np.ndarray) -> tuple[float, float, float]:
    mask = np.isfinite(n) & np.isfinite(g) & (n > 0)
    x = 1.0 / n[mask]
    y = g[mask]
    if len(x) < 3:
        return float("nan"), float("nan"), float("nan")
...
def _screening_label(rho_last: float, g_last: float) -> str:
    if np.isfinite(g_last) and g_last > 1.0:
        return "symmetry_aligned_outside_random_orientation_scale"
...
def summarize_cells(frame: pd.DataFrame, master_seed: int = 77124000) -> pd.DataFrame:
    ...
    for (family, n, k), group in frame.groupby(["family", "n", "k"], dropna=False):
        meta = {
            "family": family,
            "n": int(n),
            "k": int(k),
        ...
        rows.append(
            {
                ...
                "rho_last": rho_last,
                "loglog_slope": log_slope,
                "loglog_slope_ci95_low": slope_lo,
                "loglog_slope_ci95_high": slope_hi,
                "loglog_r2": log_r2,
                "power_decay_exponent": float(-log_slope),
                "plateau_intercept_c": plateau,
                "plateau_intercept_ci95_low": plateau_lo,
                "plateau_intercept_ci95_high": plateau_hi,
                "plateau_inv_n_slope": inv_slope,
                "plateau_r2": plateau_r2,
                "finite_size_trend": trend,
                "screening_label": _screening_label(rho_last, g_last),
                "screening_rule": "g_last>1 => symmetry-aligned; otherwise frozen rho equivalence band [0.90,1.10] distinguishes rank-typical from structured deviation",
            }
        )
    return pd.DataFrame(rows)
...
def build_scaling_analysis(
    ...
    manifest = {
        ...
        "fit_diagnostics": {
            "loglog": "OLS of log(mean orientation_growth_ratio) on log(n); bootstrap resamples circuits independently within each n cell",
            "plateau": "OLS of mean orientation_growth_ratio on [1, 1/n]; intercept is an exploratory finite-size diagnostic, not a proof of a nonzero limit",
        },
        "classification": "descriptive screen only; g_last>1 identifies a deviation larger than one Haar population-null scale after normalization, otherwise the frozen rho equivalence band [0.90,1.10] is used",
